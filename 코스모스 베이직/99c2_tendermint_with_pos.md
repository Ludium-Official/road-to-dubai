...
## 4. 장거리 공격 (Long Range Attack) 방지: 보증금 해지 기간 & 약한 주관성(Weak Subjectivity) 
장거리 공격(Long Range Attack, LRA)은 PoS 기반의 합의 알고리즘에서도 중요한 문제이다. 이는 자유로운 보증금 예치 및 인출로 인해 투표권자의 모임 구성이 시간이 지남에 따라 변경될 수 있는 특성 때문에 발생하는 공격이다. 제네시스 블록부터 새로운 체인을 생성할 수 있는 가능성을 활용하는 공격입니다. 이 공격은 PoS의 'Nothing at stake' 문제를 해결하기 위한 방안이 필요하다.
