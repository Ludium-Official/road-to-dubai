...
## 0. IBC(Inter-Blockchain Communication) 프로토콜
비트코인의 탄생은 다양한 설계 결정과 사용 사례를 가진 블록체인의 캄브리아기 폭발로 이어졌다. 이러한 분산 원장은 서로 다른 용도로 사용되지만, 의미 있는 상호 작용이 제한된 silos로 존재했고 지금도 어느 정도 존재한다. 세계 각지에 있는 다양한 유형의 컴퓨터가 서로 통신할 수 있도록 하는 인터넷과 마찬가지로, 진정한 가치 창출을 위해서는 블록체인 간의 연결 조직 역할을 하는 유사한 기술이 필요하다.

이러한 사용 사례를 여러 블록체인에서 활용할 수 없다면 블록체인의 효용성이 크게 떨어진다. 인터넷이 제공한 혁신은 정보가 전 세계 여러 지역으로 쉽게 이동할 수 있다는 점이다. 마찬가지로 다양한 블록체인의 유용성은 여러 플랫폼에서 자유롭게 액세스할 수 있어야 한다. 
- 예를 들어, 사용자는 한 블록체인의 스테이블코인을 사용하여 다른 블록체인에 존재하는 탈중앙화 거래소(DEX)의 유동성 풀(LP)을 통해 수익을 창출하고 싶을 수 있다. 

또는 다른 블록체인이 제공하는 개인 정보 보호 속성을 활용하고 싶을 수도 있다. 이러한 사용 사례 등을 실현하려면 체인 간의 상호 운용성이 필요하다. IBC는 상호 운용성 문제를 해결할 뿐만 아니라 신뢰를 최소화하고, 안전하고, 확장 가능하며, 범용적인 방식으로 이를 해결한다.

 IBC 프로토콜은 이러한 목적을 달성하는 것을 목표로 한다: 
- IBC는 서로 다른 두 블록체인이 서로 통신할 수 있도록 하는 범용 상호운용성 프로토콜이다. 
- IBC는 신뢰할 수 있고 질서정연하며 인증된 통신을 보장한다.

흔히 기존 Web 네트워크 환경에서 HTTPS와 비교하여 생각해보았을 때 IBC는 이미 신뢰를 기반으로 하고 있다는 점이 크게 다르다. 아마도 IBC의 가장 중요한 속성 중 하나는 신뢰 최소화일 것이다. 블록체인에서 신뢰 최소화의 속성은 본질적으로 보안과 관련이 있다. 완전히 '신뢰가 필요 없는' 분산 시스템은 존재하지 않는다. 
- 따라서 보안의 문제는 누가 또는 무엇을 신뢰하는가, 그리고 그 신뢰가 어떻게 침해될 수 있는가, 즉 신뢰하는 주체가 손상되기 위해서는 무엇이 필요한가라는 질문으로 귀결된다. 

이러한 신뢰적 의미에서 대부분의 브릿징 솔루션과 달리 IBC는 신뢰할 수 있는 제3자를 사용하지 않는다. 즉, 두 개의 특정 체인이 제공하는 기능(그리고 기본적으로 합의 메커니즘)을 사용하기 위해 두 체인을 신뢰한다면, IBC를 사용해 해당 체인 간에 상호작용하는 동안 추가적인 신뢰 가정이 필요하지 않는다. 

또한 IBC는 토큰 전송을 촉진하는 단순한 브릿지 그 이상이다. 이는 범용 메시지 전달 프로토콜이다. 즉, FT/NFT, 계정, 앱 체인 상태 등 모든 형태의 데이터가 IBC를 통해 전달될 수 있다.
...
## 1. 전송 계층 (TAO Layer)
TAO는 Transport, Authentication, Ordering의 의미를 포함하고 있다. 이를 간단히 전송 계층(TAO Layer)이라고 부른다. 전송 계층의 핵심 구성 요소는 라이트 클라이언트, 릴레이어, 연결, 채널이다. 

전송 계층은 IBC를 통해 전달되는 메시지는 데이터 패킷 내에서 전송된다. 그리고 전송 계층은 이러한 데이터 패킷을 전송, 인증 및 순서 지정하는 역할을 담당한다. 전송 계층은 패킷의 데이터 내용이나 수신 체인에서 패킷을 어떻게 해석해야 하는지에 대해 아무것도 지정하지 않는다. 전송 계층의 관점에서 데이터 패킷 내의 정보는 무작위 바이트에 불과한다. 


### 1. 라이트 클라이언트 (Light Client 또는 Client)
라이트 클라이언트(Light Client)는 블록체인의 경량화된 표현이다. 간단히 클라이언트(Client)라고도 한다. 풀 노드와 달리 라이트 클라이언트는 블록체인에 포함된 모든 메시지의 전체 기록을 저장하지 않는다. 트랜잭션을 실행하지도 않는다. 대신 라이트 클라이언트는 전체 노드에 연결하여 블록 헤더(블록에 포함된 데이터의 요약)를 확인하도록 설계되었다. 이로 인해 라이트 클라이언트는 저장 공간과 연산 자원 측면에서 효율적이다.

IBC를 통해 상호 작용하는 두 개의 독립적인 블록체인 A와 B가 있다고 해보자. 이들은 상대방 체인의 라이트 클라이언트를 가지고 있다. 즉, 체인 A는 자신의 블록체인에 체인 B의 블록체인을 가볍게 표현하는 역할을 하는 라이트 클라이언트를 가지고 있다. 여기서 라이트 클라이언트는 상대 블록체인의 상태를 간략하게 표현하여 검증할 수 있는 역할을 한다. 
...
IBC의 라이트 클라이언트 기반 검증 방식은 HTTPS 프로토콜과 비교해볼 때 인증 절차가 간소화된다는 이점이 있다.
- HTTPS 프로토콜: 제3자 기관을 통해 비대칭 암호화를 사용하여 인증을 수행한다. 
- IBC 프로토콜: 라이트 클라이언트를 통해 블록체인 간 직접적인 검증을 통해 신뢰를 획득한다.

이로 인해 HTTPS에 비해 IBC는 인증 과정이 간소화되지만, 통신하려는 외부 체인의 수가 증가할수록 관리해야 할 요소가 늘어나는 단점이 있다. 이는 향후 추가된 미들웨어 계층에서 해결할 수 있다. 

### 2. 릴레이어 (Relayer)
오프체인 프로세스인 릴레이어(Relayer)는 블록체인 간의 메시지 전달을 지속적으로 관찰하고 중계하는 역할을 한다:
- 릴레이어는 권한이 없는 프로세스이므로 누구나 실행할 수 있다.
- 릴레이어는 체인 A에서 체인 B로 메시지를 커밋한 것을 상태 머신에서 확인하면, 이를 받아 체인 B에게 전달한다.

릴레이어는 패킷 검증을 수행하지 않으므로 신뢰할 필요가 없지만, 패킷 제출을 통한 IBC 네트워크 Liveness 외에도 IBC 설정에서 릴레이어는 특히 중요한 역할을 담당한다. 
...
### 3. 연결 (Connection)
연결(Connection)은 각 체인에 있는 라이트 클라이언트를 통해 설정된다. 위의 예시를 보면 체인 A의 연결은 체인 B의 라이트 클라이언트를 사용하고, 체인 B의 연결은 체인 A의 라이트 클라이언트를 사용한다. 연결을 통해 두 체인은 서로의 상태를 확인하고 동기화할 수 있다.
...
- 예를 들어, ICS 20이라고 하는 토큰 전송을 위한 IBC 수준의 애플리케이션은 데이터 패킷을 구조화하는 방법과 수신 체인에서 이를 해석하는 방법을 명시하고 있다. 
...
IBC를 이해하는 데 도움이 될 수 있는 간단한 비유는 우편 배달 시스템이다. 
...
3. 그러면 수신자는 해당 봉투를 열어 편지를 읽는다. 
	1. IBC의 전송 계층은 우편 서비스로 생각할 수 있다. 
	2. 우편 서비스는 편지의 내용이나 수신자가 메시지를 어떻게 해석해야 하는지 알려주지 않으며, 봉투의 내용물도 알지 못한다. 
	3. 봉투 자체는 한 체인에서 다른 체인으로 전송되는 IBC 패킷으로 생각할 수 있으며, A 지점에서 봉투를 수거하여 B 지점으로 보내는 작업만 수행한다. 
