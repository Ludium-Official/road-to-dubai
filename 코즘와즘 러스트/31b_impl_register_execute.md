...
msg에 등록된 name 을 인자로 받아 이를 컨트랙트 내부 상태인 `NameRecord`에 등록하는 로직을 작성한다. 
`Map<Key,Value>` 형태로 String으로 입력된 이름을 byte로 크기르 줄어 key값으로 자신의 계정 주소를 value로 매핑하여 저장한다. name은 고유해야 하기 떄문에 

이제 `execute_register` 함수를 작성한다. msg에 등록된 name을 인자로 받아 이를 컨트랙트 내부 상태인 `NameRecord`에 등록한다. `Map<Key, Value>` 형태로 `String으`로 입력된 이름을 `byte`로 변환하여 `key`로 사용하고, 그 이름을 등록한 계정 주소를 value로 매핑하여 저장한다. 이름은 고유해야 하기 때문에 중복된 이름은 등록할 수 없다:
```rust 
...
이제 중복된 이름을 등록하려고 할 때 발생하는 에러를 처리하도록 `execute_register` 함수를 업데이트해보자. 에러 반환타입을 `ContractError`로 변경해주고, 이미 등록된 이름에 대해서 재등록이 불가능하도록 비즈니스 로직을 추가해준다:
```rust
#[cfg_attr(not(feature = "library"), entry_point)]
pub fn execute(
    deps: DepsMut,
